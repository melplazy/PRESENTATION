...
## It works, but biomarkers ... {.flexbox .vleft .build}
...
* if $\beta\neq0$, to detect a joint effect of $Z$:  
    $$\beta\gamma+\alpha\neq0$$
    (@chen_sample_2011);
...
for (igroup in all_gif[["group"]]) {
  ...
  animate(
    ...
    renderer = av_renderer(
      file = paste0("./images/", gsub("## \\$\\\\([^$]*)\\$.*", "\\1", igroup), "_RMSE.mp4")
    )
  )
  ...
  animate(
    ...
    bg = ggplot2::theme_get()$plot.background$colour,
    renderer = av_renderer(
      file = paste0("./images/", gsub("## \\$\\\\([^$]*)\\$.*", "\\1", igroup), "_bias.mp4")
    )
  )
  ...
  animate(
    ...
    bg = ggplot2::theme_get()$plot.background$colour,
    renderer = av_renderer(
      file = paste0("./images/", gsub("## \\$\\\\([^$]*)\\$.*", "\\1", igroup), "_var.mp4")
    )
  )
...
```{r effect_size3, out.height = "432px", out.width = "768px"}
ggplot(
  data = jm_data$p.estimateJM.sign$data %>% 
    filter(InGWAS) %>% 
...
```{r effect_size4, out.height = "432px", out.width = "768px"}
ggplot(
  data = jm_data$p.estimateJM.sign$data %>% 
    filter(InGWAS) %>% 
    filter(GeneSymbol.Closest%in%c("TCF7L2", "MTNR1B", "G6PC2")) %>% 
...
```{r, eval = FALSE}
jm_data$p.estimateJM.sign$data %>% 
  filter(RSID%in%c("rs10830963", "rs17747324")) %>% 
  select(
    RSID, RiskAllele, strand,
