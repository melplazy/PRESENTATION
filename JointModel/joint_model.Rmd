...
devtools::source_url('https://github.com/mcanouil/DEV/raw/master/Rscripts/jointModelSimulation.R')
...
#   to = "Routput/MetaboChip_T2D.annot.Rdata"
# )
load(file = "Routput/MetaboChip_T2D.annot.Rdata")
...
#   envir = jm_data
# )
# save(jm_data, file = "Routput/DataArticle_20180417.Rdata")
load(file = "Routput/DataArticle_20180417.Rdata")
...
```{r, out.height = "432px", out.width = "768px"}
...
```{r, out.height = "432px", out.width = "432px"}
...
## Let's start with the Cox Model ... {.flexbox .vleft}
...
* if $\beta\neq0$  
    (to detect a joint effect of $Z$: $\beta\gamma+\alpha\neq0$, @chen_sample_2011);
...
## Hypothesis testing {.flexbox .vleft}
...
## A more "naive" approach to compare to! {.flexbox .vleft}
...
oneSimulation <- function(sampleSize, settings, seed) {
  ...
  smldta <- smldta %>% 
    ...
    mutate(
      ...
      Ytrue_event = ifelse(group&DiscreteEventTime==Time, rnorm(n = 1, mean = 7.5, sd = jm_data$rawsettings$sigma), Ytrue),
...
simulated_data <- mclapply(X = 1:50, mc.cores = 50, FUN = function(iseed) {
  oneSimulation(sampleSize = 100, settings = jm_data$rawsettings, seed = 20181018+iseed)
}) %>% 
  bind_rows()
...
p <- simulated_data %>% 
  ...
  ggplot() +
    ...
    labs(y = "Yobs", title = "N=100; Seed: {closest_state}") +
...
animate(
  ...
  renderer = gifski_renderer(
    file = "./images/simulate_data.gif"
  )
)
```


## What could we expect with those parameters? {.flexbox .vleft}
...
## What about computationtime? {.flexbox .vcenter}
...
p <- ggdata_clean %>% 
  ...
  ggplot(aes(x = x_pos, y = y_pval, colour = colour)) +
    ...
    transition_reveal(id = id, along = y_reveal, range = c(0, 90))
