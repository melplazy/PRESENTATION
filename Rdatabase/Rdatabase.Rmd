...
<img src="images/dbplyr.png" height="250" class="auto-fadein"></img>
</div>
...
* Connaître les dimmensions de l'objet avant `collect()` ?
    ```{r}
    dim(ratings_summary_db)
    ```
    ```{r}
    nrow(ratings_summary_db)
    ```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height = 3, fig.width = 2}
con <- dbConnect(
  drv = odbc(),
  driver = "PostgreSQL ODBC Driver(UNICODE)",
  server = "localhost",
...
ratings_db <- tbl(con, "ratings")
ratings_db %>% 
  dbplot_boxplot(data = ratings, x = Year, var = IMDbRating)
```
