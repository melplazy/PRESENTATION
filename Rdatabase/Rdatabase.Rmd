...
__Note :__ Raccourci `Ctrl+M` dans Rstudio pour faire apparaître un "pipe".
...
* Trouvez les films vus entre le 25 avril 2018 et 3 mai 2018.
...
* Effectuez la jointure de `planes`et `flights` pour identifier les trois constructeurs ayant le plus grand nombre d'heures de vol (`?dplyr::join`).  
...
ratings_agg <- read_csv("materials/ratings.csv") %>%
  ...
  right_join(
    tibble(
      "Date Rated" = seq(from = as.Date("2015-01-01"), to = Sys.Date(), by = 1)
    )
  ) %>%
  mutate(
    Year = lubridate::year(`Date Rated`),
...
<img src="images/Coeos_IMDb-3.png"></img>
...
* *nodbi* sur GitHub : [nodbi](https://github.com/ropensci/nodbi)
...
# Merci ! | Présentation disponible au format Rmarkdown sur {.flexbox .vcenter}

<a href = "https://github.com/mcanouil/PRESENTATION/tree/master/Rdatabase"><img src="images/GitHub_Logo.png" height="250" class="auto-fadein"></img></a>


<!-- DROP DATABASE test_data; DROP ROLE test_user; DROP DATABASE datawarehouse; DROP ROLE data_user; DROP DATABASE movies; DROP ROLE imdb_user; -->
